...
resource "yandex_loadtesting_agent" "todoops" {
  ...
  compute_instance {
    ...
    network_interface {
      ...
      nat                = true
      security_group_ids = [yandex_vpc_security_group.all_outbound.id]
    }
  }
